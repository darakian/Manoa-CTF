...
void main(int argc, char *argv[])
{
  ...
  remote_sockaddr.sin_port = htons(9003);
  ...
  if (bind(server_socket,(struct sockaddr *)&server_address, sizeof(server_address)) < 0)
    ...
    char winrar[] = "You a winrar!";
    char hash[64];
    char base64_buffer[88];
    int write_length = 0;
    sodium_memzero(&hash, sizeof(hash));
    crypto_generichash(hash, sizeof(hash), winrar, sizeof(winrar),NULL, 0);
    write_length = base64_encode(hash, sizeof(hash), base64_buffer, sizeof(base64_buffer));

    write(server_socket, hash, sizeof(hash));
    close(server_socket);
}
